...
import { ContactImport } from '../../components/shared/ContactImport'
...
export default function Contacts() {
  ...
  function openAssign(c) {
    setAssignContact(c)
    const ids = (c.group || []).map((g) => (typeof g === 'object' ? g._id : g)).filter(Boolean)
  ...
  async function addContact() {
    ...
    try {
      ...
      const { data } = await contactsApi.create({ ...form, phone: normalizedPhone })
      if (data.success) {
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400">
            Manage subscribers ·{' '}
            <Link to="/contacts/groups" className="text-[#25D366] hover:underline">
              Groups
            </Link>
          </p>
        </div>
                ...
                <TR key={c._id}>
                  ...
                  <TD>
                    <button
                      ...
                      className={`text-xs font-medium ${c.optedOut ? 'text-[#EF4444]' : 'text-[#34D399]'}`}
                    >
                      {c.optedOut ? 'Yes' : 'No'}
                    </button>
                  </TD>
                  <TD>
                    <Button type="button" size="sm" variant="ghost" onClick={() => openAssign(c)}>
                      Assign
                    </Button>
                  </TD>
                  <TD>
                    <Button type="button" size="sm" variant="danger" onClick={() => removeContact(c._id)}>
                      Delete
                    </Button>
                  </TD>
                </TR>
      ...
      >
        <p className="text-sm text-slate-400 mb-3">
          Select one or more groups for {assignContact?.name || 'this contact'}.
        </p>
        <select
          multiple
          className="w-full min-h-[140px] rounded-lg border border-[#334155] bg-[#0F172A] px-3 py-2 text-sm text-[#F1F5F9]"
          value={selectedGroups.map((id) => String(id))}
          onChange={(e) => {
