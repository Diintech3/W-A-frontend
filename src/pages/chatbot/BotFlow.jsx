...
export default function BotFlow() {
    ...
    async function load() {
      ...
      try {
        ...
        if (data.success) {
          ...
          setTriggerKeyword(f.triggerKeyword || 'hi')
          setNodes(f.nodes?.length ? f.nodes : [newNode(0)])
        }
      } catch (e) {
        toast.error(e.response?.data?.message || 'Failed to load flow')
      } finally {
        setLoading(false)
      }
    }
  ...
  function addOption(nodeIdx) {
    setNodes((prev) => {
      const n = [...prev]
      const opts = [...(n[nodeIdx].options || [])]
      opts.push({ label: 'Option', value: '1', nextNodeId: '' })
      n[nodeIdx] = { ...n[nodeIdx], options: opts }
      return n
    })
  }
  ...
  async function save() {
    ...
    try {
      ...
      if (data.success) toast.success('Flow saved')
    ...
    <div className="space-y-6 max-w-4xl">
      <div>
        ...
        <p className="text-sm text-slate-400">
          Keyword triggers the first node. Menu nodes wait for matching option label/value.
        </p>
      </div>
        ...
        {nodes.map((node, idx) => (
          <Card key={node.id} title={`Node ${idx + 1}`}>
            <div className="space-y-3">
              ...
              <label className="block w-full">
                <span className="mb-1 block text-sm text-slate-300">Node id</span>
                <input
                  className="w-full rounded-lg border border-[#334155] bg-[#0F172A] px-3 py-2 text-sm text-slate-400"
                  value={node.id}
                  readOnly
                />
              </label>
              <label className="block w-full">
                ...
                <textarea
                  className="w-full min-h-[80px] rounded-lg border border-[#334155] bg-[#0F172A] px-3 py-2 text-[#F1F5F9]"
                  value={node.content}
                  onChange={(e) => updateNode(idx, { content: e.target.value })}
                />
              </label>
              <Input
                label="Next node id (optional)"
                value={node.nextNodeId}
                onChange={(e) => updateNode(idx, { nextNodeId: e.target.value })}
              />
              {node.type === 'menu' && (
                <div className="rounded-lg border border-[#334155] p-3 space-y-2">
                  <div className="flex justify-between items-center">
                    ...
                    <Button type="button" className="!py-1 !px-2 text-xs" onClick={() => addOption(idx)}>
                      Add option
                    </Button>
                  </div>
                  {(node.options || []).map((opt, oi) => (
                    <div key={oi} className="grid gap-2 sm:grid-cols-3">
                      ...
                      <Input
                        ...
                        onChange={(e) => updateOption(idx, oi, { nextNodeId: e.target.value })}
                      />
                    </div>
                  ))}
                </div>
      ...
      <Card title="Preview">
        <ol className="list-decimal space-y-2 pl-5 text-sm text-slate-300">
          <li>
            User sends “{triggerKeyword}” → first node fires.
          </li>
          {nodes.map((n, i) => (
            <li key={n.id}>
              <span className="text-[#25D366]">{n.type}</span>: {n.content?.slice(0, 80) || '…'}
              {n.type === 'menu' && (n.options || []).length > 0 && (
                <ul className="mt-1 list-disc pl-5 text-slate-400">
                  {(n.options || []).map((o) => (
                    <li key={o.value}>
                      {o.label} → {o.nextNodeId || 'end'}
                    </li>
                  ))}
                </ul>
              )}
